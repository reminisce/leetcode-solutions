 ...
 * profit. You may complete at most k transactions.
 *
 * Note:
...
public class BestTimeToBuyAndSellStockIV {
    ...
    public static void main(String[] args) {
        ...
        BestTimeToBuyAndSellStockIV app = new BestTimeToBuyAndSellStockIV();
        System.out.println(app.maxProfit(k, prices));
    }

    /**
     *
     * Define two variables for dp:
     * global[i][j] denotes the max profit to the i-th day
     * with maximum j transactions.
     * local[i][j] denotes the max profit to sell on the
     * i-th day with maximum j transactions.
     *
     * global[i][j] = max(global[i-1][j], local[i][j]) Eq. (1)
     * local[i][j] = max(local[i-1][j]+prices[i]-prices[i-1],
     *                   global[i-1][j-1]+max(prices[i]-prices[i-1], 0)) Eq. (2)
     * The first term on the right side of Eq. (2) is
     * local[i-1][j]+prices[i]-prices[i-1]
     * rather than
     * local[i-1][j-1]+prices[i]-prices[i-1]
     * is because local[i-1][j] means we sell on the (i-1)-th day
     * with prices[i-1] with maximum j transactions. If we change
     * it the last transaction to the i-th day by adding
     * prices[i]-prices[i-1], the total number of transactions
     * is still j.
     * @param k
     * @param prices
     * @return
    ...
    public int maxProfit(int k, int[] prices) {
        if (k >= prices.length/2) return maxProfitUnlimitedK(prices);
    ...
    /**
     * This becomes stock problem two
     * because of k >= prices.size().
     */
    private int maxProfitUnlimitedK(int[] prices) {
        int p = 0;
        for (int i = 1; i < prices.length; ++i) {
            p += Math.max(prices[i]-prices[i-1], 0);
        }
        return p;
    }
}
